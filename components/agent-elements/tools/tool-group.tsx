...
import { getToolStatus, toolInputRecord, toolOutputRecord } from "../utils/format-tool";
...
function summarizeNestedTools(nestedTools: AgentToolPart[]): string {
...
function getNestedCounts(nestedTools: AgentToolPart[]) {
  const fileTypes = new Set(["tool-Read", "tool-Edit", "tool-Write"]);
  const searchTypes = new Set([
    "tool-Search",
    "tool-Grep",
...
export const ToolGroup = memo(function ToolGroup({
  ...
  const inPart = toolInputRecord(part);
  const outPart = toolOutputRecord(part);
  const description = String(inPart.description ?? "");
  const [elapsedMs, setElapsedMs] = useState(0);
  const [expanded, setExpanded] = useState(defaultOpen ?? false);
  const [visibleCount, setVisibleCount] = useState(0);
  const meta = part.callProviderMetadata as
    | { custom?: { startedAt?: number } }
    | undefined;
  const startedAt =
    meta?.custom?.startedAt ?? part.startedAt;
  const hasNestedTools = nestedTools.length > 0;
  const streamKey = part.toolCallId ?? part.id ?? "";
  const outputDurationMs =
    (typeof outPart.totalDurationMs === "number"
      ? outPart.totalDurationMs
      : undefined) ??
    (typeof outPart.duration === "number" ? outPart.duration : undefined) ??
    (typeof outPart.duration_ms === "number" ? outPart.duration_ms : undefined);
  ...
  const elapsedTimeDisplay = formatElapsedTime(
    !isPending && outputDurationMs != null ? outputDurationMs : elapsedMs,
  );
