...
import { getToolStatus, areToolPropsEqual, toolInputRecord, toolOutputRecord } from "../utils/format-tool";
...
export const TodoTool = memo(function TodoTool({
  ...
  const { oldTodos, newTodos } = useMemo(() => {
    const out = toolOutputRecord(part);
    const inp = toolInputRecord(part);
    const oldTodosRaw = out.oldTodos;
    const newTodosRaw = inp.todos ?? out.newTodos;
    const oldTodos: TodoItem[] = Array.isArray(oldTodosRaw)
      ? (oldTodosRaw as TodoItem[])
      : [];
    const newTodos: TodoItem[] = Array.isArray(newTodosRaw)
      ? (newTodosRaw as TodoItem[])
      : [];
