...
import {
  toolInputRecord as inDict,
  toolOutputRecord as outDict,
...
export type ToolMeta = {
  ...
  title: (part: AgentToolPart) => string;
  subtitle?: (part: AgentToolPart) => string;
  variant: ToolVariant;
};
...
export const toolRegistry: Record<string, ToolMeta> = {
  "tool-Task": {
    ...
    title: (part) => {
      ...
      const subagentType = str(inDict(part).subagent_type) || "Agent";
      return isPending
        ? `Running ${subagentType}`
        : `${subagentType} completed`;
    },
    subtitle: (part) => {
      const desc = str(inDict(part).description);
  ...
  "tool-Skill": {
    icon: Sparkles,
    title: () => "Skill",
    subtitle: (part) => str(inDict(part).skill),
    variant: "simple",
  },
  "tool-Grep": {
    icon: Search,
    title: (part) => {
      ...
      const numFiles = num(outDict(part).numFiles);
      return numFiles > 0 ? `Grepped ${numFiles} files` : "No matches";
    },
    subtitle: (part) => {
      const pattern = str(inDict(part).pattern);
      const path = str(inDict(part).path);
      if (path) {
  ...
  "tool-Read": {
    ...
    subtitle: (part) => {
      const filePath = str(inDict(part).file_path);
      if (!filePath) return "";
  ...
  "tool-Edit": {
    ...
    subtitle: (part) => {
      const isPending =
        part.state !== "output-available" && part.state !== "output-error";
      if (isPending) return "";
      const oldString = str(inDict(part).old_string);
      const newString = str(inDict(part).new_string);
  ...
  "tool-Bash": {
    ...
    subtitle: (part) => {
      const command = str(inDict(part).command);
      if (!command) return "";
  ...
  "tool-WebSearch": {
    ...
    subtitle: (part) => {
      const query = str(inDict(part).query);
  ...
  "tool-PlanWrite": {
    ...
    title: (part) => {
      const isPending =
        part.state !== "output-available" && part.state !== "output-error";
      const action = str(inDict(part).action) || "create";
      if (isPending) {
        if (action === "create") return "Creating plan";
        if (action === "approve") return "Approving plan";
        return "Updating plan";
      }
      const planRaw = inDict(part).plan;
      const planObj =
        planRaw && typeof planRaw === "object" && planRaw !== null
          ? (planRaw as { status?: unknown })
          : undefined;
  ...
  "tool-cloning": {
    ...
    subtitle: (part) => str(inDict(part).repo),
    variant: "simple",
  },
  "tool-Thinking": {
    icon: Sparkles,
