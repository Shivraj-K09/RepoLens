...
export function mapToolInvocationToStep(
  ...
  if (
    ...
    result !== null &&
    Array.isArray((result as { structuredPatch?: unknown }).structuredPatch)
  ) {
    const patches = (result as { structuredPatch: Array<{ lines?: string[] }> })
      .structuredPatch;
    step.diffStats = calculateDiffStatsFromPatch(patches);
    step.diffLines = getDiffLinesFromPatch(patches);
  }
