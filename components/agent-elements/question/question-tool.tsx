...
export type QuestionToolPart = AgentToolPart & {
...
function QuestionToolInner({ part }: QuestionToolProps) {
  const input = part.input as QuestionToolInput | undefined;
  const [localIndex, setLocalIndex] = useState(input?.questionIndex ?? 1);
  const questions: QuestionConfig[] = input?.questions ?? [];
  const totalQuestions = input?.totalQuestions ?? questions.length;
  const isControlled = typeof input?.questionIndex === "number";
  ...
  const outputAnswer = (() => {
    ...
    const a = (o as { answer?: QuestionAnswer }).answer;
    return a;
  })();
      ...
      {isComplete ? (
        ...
        <QuestionPrompt
          ...
          onSubmit={(nextAnswer) => {
            ...
            input?.onSubmitAnswer?.(nextAnswer);
...
export function QuestionTool(props: QuestionToolProps) {
  const key = props.part.toolCallId ?? "question-tool";
  return <QuestionToolInner key={key} {...props} />;
}
