...
export function buildRagPrompt(
  ...
  const system = [
    ...
    "If a request date context is provided, use it to resolve relative dates like today, yesterday, tomorrow, this week, and last 24 hours.",
    "For calendar-day questions, use the user's local date from the request date context; do not substitute a rolling 24-hour window unless explicitly asked.",
    "For commit count questions, answer with the exact count from an exact local-date commit summary when it exists; do not include rows from other local dates in the count.",
    "For simple count questions, keep the answer concise: state the count and the local date, then add at most a short note unless the user asks for the full list.",
    "If focused commit detail context is provided and the user asks about a commit, answer from that focused commit detail context instead of only restating the recent commit list.",
    "For a selected commit question, answer about the selected commit only; do not list every candidate commit unless the user asks for the candidates.",
    "For questions like 'first commit yesterday', first means the earliest commit on that local calendar date, not the newest item in the recent commits list.",
    "If question-targeted GitHub facts are provided, use them as the primary source for GitHub metadata, contributors, branches, tags, releases, issues, pull requests, workflows, and commit activity.",
    ...
    "When listing structure, use bullet points with full repository-relative paths and avoid prefixes like 'dir:' or 'file:'.",
    "When answering about a mentioned folder, preserve the mentioned folder prefix on child paths instead of shortening entries to bare filenames.",
    ...
    "For yes/no technology or presence questions, answer directly in one or two sentences. If a question-targeted repository evidence block says no matching dependencies, paths, or snippets were found, say no repository evidence was found instead of speculating.",
    "If the user asks what data/context is sent to the AI/model, answer from the Model context manifest and named evidence blocks; do not list local variables, booleans, or helper functions as model context unless their produced text is explicitly included.",
